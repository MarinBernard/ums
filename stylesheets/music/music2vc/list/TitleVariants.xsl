...
<xsl:stylesheet version="3.0"
	...
	<xsl:template name="LT_TitleVariants">
		...
		<xsl:call-template name="showDebugMessage">
			<xsl:with-param name="Template" select="'LT_TitleVariants'"/>
		...
		<xsl:call-template name="showDebugMessage">
			<xsl:with-param name="Template" select="'LT_TitleVariants'"/>
			<xsl:with-param name="Message" select="concat('Parameter $Label_Subtitle has value: ', $Label_Subtitle)"/>
		</xsl:call-template>
		<xsl:call-template name="showDebugMessage">
			<xsl:with-param name="Template" select="'LT_TitleVariants'"/>
		...
		<xsl:call-template name="showDebugMessage">
			<xsl:with-param name="Template" select="'LT_TitleVariants'"/>
			<xsl:with-param name="Message" select="concat('Parameter $Suffix_Subtitle has value: ', $Suffix_Subtitle)"/>
		</xsl:call-template>
		<xsl:call-template name="showDebugMessage">
			<xsl:with-param name="Template" select="'LT_TitleVariants'"/>
		...
		<xsl:variable name="_preferedLanguageVariant">
			<xsl:for-each select="umsb:titleVariant[@lang = $config.variants.preferredLanguage]">
				<xsl:call-template name="RT_TitleVariant">
					...
					<xsl:with-param name="Label_Subtitle" select="$Label_Subtitle"/>
					<xsl:with-param name="Suffix_Full" select="$Suffix_Full"/>
					<xsl:with-param name="Suffix_Sort" select="$Suffix_Sort"/>
					<xsl:with-param name="Suffix_Subtitle" select="$Suffix_Subtitle"/>
